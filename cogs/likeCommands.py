...
class LikeCommands(commands.Cog):
    ...
    async def like_command(self, ctx: commands.Context, uid: str):
        ...
        if not await self.check_channel(ctx):
            msg = "⚠️ This command is restricted in this channel.\n🔧 Please use it in an authorized channel."
            if is_slash:
        ...
        if not uid.isdigit() or len(uid) < 6:
            await ctx.reply("Invalid UID. It must contain only numbers and be at least 8 characters long.", mention_author=False, ephemeral=is_slash)
            return
        ...
        try:
            async with ctx.typing():
                async with self.session.get(f"{self.api_host}/like?uid={uid}", headers=self.headers) as response:
                    ...
                    embed.set_footer(text="Generated by Nibir")
                    embed.description += "\n\n➡️ **Support:** https://discord.gg/CAnmCAJPqB"
